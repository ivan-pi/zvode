...
class ZVODEResult(dict):
    ...
    # No __str__: dict.__str__ is object.__str__, which delegates to __repr__,
    # so str() and repr() already render identically (as in SciPy's results).
...
def _zvode_adaptive(
    ...
    # Python fallback (ZVODE_BACKEND=python). Production paths use drive_adaptive.
    with ZVODE_LOCK:
...
def _zvode_knots(fun, jac, y0, tspan, itol, rtol, atol, mf, iopt, zwork, rwork, iwork):
    ...
    # Python fallback (ZVODE_BACKEND=python). Production paths use drive_knots.
    with ZVODE_LOCK:
